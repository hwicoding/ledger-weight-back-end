...
- [x] GitHub Actions 워크플로우 파일 업데이트
  ...
  - [x] 서버 담당자 요청사항 반영 완료
- [x] GitHub Actions Tailscale 연동 추가
  - [x] Tailscale 연결 단계 추가 (Auth Key 방식)
  - [x] NAS 서버 호스트를 Tailscale IP (100.88.40.125)로 변경
  - [x] 포트를 22로 고정
  - [x] TS_AUTH_KEY Secret 사용
- [x] ImportError 수정 (서버 담당자 요청)
  - [x] `app/utils/constants.py`에 누락된 상수 추가
    - [x] `MIN_PLAYERS`, `MAX_PLAYERS` 추가
    - [x] `WS_MAX_CONNECTIONS`, `WS_HEARTBEAT_INTERVAL` 추가
  - [x] Import 테스트 완료
